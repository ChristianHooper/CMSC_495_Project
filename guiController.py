...
def init():
    ...
    grid_y = [row_n * (ds.SCREEN_SIZE[sc['screen_size']][1]/ (ds.GUI_GRID[3]+1)) for row_n in range(ds.GUI_GRID[3]+1)] # Divides screen length into grid
    grid_x = [cols_n * (ds.SCREEN_SIZE[sc['screen_size']][1]/ (ds.GUI_GRID[3]+1)) for cols_n in range(ds.GUI_GRID[3]+1)] # Divides screen width into grid
